...
//Returns the pulsewidth for the steering servo
...
//Uses the % the throttle is held down to calculate the proper pulsewidth 
...
//The way the car works it needs to go reverse, neutral, reverse to go backwards
void set_up_reverse() {
...
//Subcribing to throttleandsteering, publishing on encoder_speed
...
//The ISR for each of the different wheel encoders
void FLencoderISR() {
  FL_encoder_count++;
}
...
//Similar to setup in the main of the .cpp files
void setup() {
  nh.initNode();
  nh.advertise(encoder_speed); // advertise on encoder_speed
  nh.subscribe(targetcontrols); //subscribe to targetcontrols
...
void loop() {
  ...
  if (dt > speed_update_period) {
	...
	//Create the different components of the message and then publish
